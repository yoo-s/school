...
// toggle to see modifications for best fit algorithm
#define BEST_FIT
...
#ifdef BEST_FIT
/*
 * Allocate a slob block within the candidate best fit page best.
 */
static void *slob_bf_page_alloc(struct bf_page *best, size_t size, int align)
{
	struct slob_page *sp = best->sp;

	slob_t *next, *aligned = NULL;
	slob_t *prev = best->prev;
	slob_t *cur = best->cur;
	int delta = 0, units = SLOB_UNITS(size);
 ...
 */
static void *slob_best_fit(struct slob_page *sp, struct bf_page *best, size_t size, int align)
{
	slob_t *prev, *cur, *aligned = NULL;
	int delta = 0, units = SLOB_UNITS(size);
	...
	for (prev = NULL, cur = sp->freelist; ; prev = cur, cur = slob_next(cur)) {
		...
		// update best fit
		if (avail >= units + delta && best->delta > avail - (units + delta)) { /* room enough? */
			best->sp = sp;
			best->prev = prev;
			best->cur = cur;
		...
		// end loop once we reach last slob
		if (slob_last(cur))
			break;
	}
}
...
static void *slob_alloc(size_t size, gfp_t gfp, int align, int node)
{
...
#ifdef BEST_FIT
	// check for best fit
	slob_best_fit(sp, &best, size, align);
	if (best.sp != NULL && first_fit == 0) {
		first_fit = best.delta;
	}
	prev = sp->list.prev;
	...
	// if fit page is found, try to allocate page
	printk("First fit: %d\n", first_fit);
	printk("Best fit: %d\n\n", best.delta);
	
	if (best.sp != NULL) {
		if ((b = slob_bf_page_alloc(&best, size, align)) && prev != slob_list->prev && slob_list->next != prev->next) {
...
asmlinkage unsigned long sys_get_mem_used(void) {
	return mem_used;
}
