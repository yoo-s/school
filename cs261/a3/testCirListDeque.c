...
int main(int argc, char* argv[]) {
	...
	printf("Done.\n");
	printf("Checking if first element is 2...It is %g\n", frontCirListDeque(deq));
	printf("Checking if last element is 9...It is %g\n", backCirListDeque(deq));
	printf("Now printing the whole circular deque...");
	printCirListDeque(deq);
	printf("\nReversing the circular deque...");
	reverseCirListDeque(deq);
	printCirListDeque(deq);
	printf("\nLet's remove the values from the circular deque.\n");
