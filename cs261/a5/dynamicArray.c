...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
	...
	if (rightIdx < max) {
		mini = _smallerIndexHeap(heap, leftIdx, rightIdx, compare);
		if ((*compare)(heap->data[mini], heap->data[pos]) == -1) {
			swapDynArr(heap, mini, pos);
			_adjustHeap(heap, max, mini, compare);
		}
	} else if (leftIdx < max) {
